...
describe('<xhr-simple-request>', function() {
  async function basicFixture() {
    return (await fixture(`<xhr-simple-request></xhr-simple-request>`));
  }
    ...
    describe('Basics', () => {
      ...
      it('Reports aborted request', (done) => {
        element.addEventListener('api-response', function clb(e) {
          element.removeEventListener('api-response', clb);
          assert.equal(e.detail.id, 'request' + lastId);
          assert.isTrue(e.detail.isError);
          assert.equal(e.detail.error.message, 'Request aborted');
          done();
        });
        const request = {
          url: APIROOT,
